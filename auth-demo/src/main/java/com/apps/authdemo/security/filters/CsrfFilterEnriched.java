 ...
 * {@link CsrfFilterEnriched} is invoked for any request that allows state to change. Typically
 ...
 * @since 3.2
 */
public final class CsrfFilterEnriched extends OncePerRequestFilter {
	/**
	...
	private final CsrfTokenRepository tokenRepository = new LazyCsrfTokenRepository(
			new HttpSessionCsrfTokenRepository());;
	private RequestMatcher requireCsrfProtectionMatcher = DEFAULT_CSRF_MATCHER;
	private AccessDeniedHandler accessDeniedHandler = new AccessDeniedHandlerImpl();
	private SocketIOServer server;

	public CsrfFilterEnriched(SocketIOServer server) {
		this.server = server;
	}
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		if (!csrfToken.getToken().equals(actualToken)) {
			notifySocketIo(actualToken, csrfToken, "FAILED");
		...
		notifySocketIo(actualToken, csrfToken, "SUCCESS");
		filterChain.doFilter(request, response);
	}
